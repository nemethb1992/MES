...
<%
	...
	try {
		...
		abasConnection = AbasObjectFactory.INSTANCE.openAbasConnection(user.getUsername(), user.getPassword(),
				of.getLocale(), new AppBuild(request).isTest());
...
%>
<div class="modal fade my-fade" id="interrupt-level1" tabindex="-1"
	...
	<div class="modal-dialog modal-lg modal-dialog-centered"
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="form-group row">
					<label for="example-search-input" class="col-3 col-form-label">Hiba
						típusa</label>
					<div class="col-9">
						<select onmousedown="suspendListEvent(this)" id="failure-list"
							name="failure-list"
							class="custom-select d-block w-100 failure-list" required>
							<option value="">Válasszon...</option>
							<option class='opt'
								value='<%=of.getWord(DictionaryEntry.SUSPEND_LIST_ITEM_1)%>'><%=of.getWord(DictionaryEntry.SUSPEND_LIST_ITEM_1)%></option>
							<option class='opt'
								value='<%=of.getWord(DictionaryEntry.SUSPEND_LIST_ITEM_2)%>'><%=of.getWord(DictionaryEntry.SUSPEND_LIST_ITEM_2)%></option>
							<option class='opt'
								value='<%=of.getWord(DictionaryEntry.SUSPEND_LIST_ITEM_3)%>'><%=of.getWord(DictionaryEntry.SUSPEND_LIST_ITEM_3)%></option>
							<option class='opt'
								value='<%=of.getWord(DictionaryEntry.SUSPEND_LIST_ITEM_4)%>'><%=of.getWord(DictionaryEntry.SUSPEND_LIST_ITEM_4)%></option>
							<option class='opt'
								value='<%=of.getWord(DictionaryEntry.SUSPEND_LIST_ITEM_5)%>'><%=of.getWord(DictionaryEntry.SUSPEND_LIST_ITEM_5)%></option>
						</select>
					</div>
				</div>
				<div class="form-group row">
			...
			<div class="modal-footer">
				...
				<%
					if (suspendWithoutApproval) {
							out.print("<button type='button' class='btn btn-primary w-25' onclick='SuspendTask(false)'>"
									+ of.getWord(DictionaryEntry.NEXT) + "</button>");
						} else {
							out.print("<button type='button' class='btn btn-primary w-25' onclick='InterruptTask()'>"
									+ of.getWord(DictionaryEntry.NEXT) + "</button>");
						}
				%>
			</div>
...
<div class="modal fade my-fade" id="interrupt-level2" tabindex="-1"
	role="dialog" aria-labelledby="exampleModalCenterTitle"
	aria-hidden="true">
	<div class="modal-dialog modal-lg modal-dialog-centered"
		role="document">
		<div class="modal-content">
			...
			<div class="modal-body pt-1">
				<div class="form-group row">
					<label for="example-search-input" class="col-12 col-form-label"><%=of.getWord(DictionaryEntry.DISRUPTION_REASON)%></label>
					<div class="col-12">
						<textarea class="form-control error-text-back" disabled
				...
				<div class="form-group row">
					...
					<div class="col-9">
						<input class="form-control username-input" type="text" value=""
							id="username-input">
					</div>
				</div>
				<div class="form-group row">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary w-25"
					data-dismiss="modal" onclick='ResumeTask()'><%=of.getWord(DictionaryEntry.CANCEL)%></button>
