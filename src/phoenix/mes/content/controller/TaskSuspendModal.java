...
public class TaskSuspendModal extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		boolean secureSuspend = true;
		if(suspendType.equals("operator")) {
			WorkCenter.Details workCenter = (WorkCenter.Details) session.getAttribute("WorkCenter");
			secureSuspend = workCenter.getSuspendWithoutApproval();
		}
		...
		request.setAttribute("SecureSuspend", secureSuspend);
		request.setAttribute("workSlipId", task.getWorkSlipId().toString());
		String encodedURL = response.encodeRedirectURL("/Views/Partial/TaskSuspendModal.jsp");
